...
function defaultLegacyDeployOutputPath(cwd, network) {
  if (network === "localhost") {
    return path.resolve(cwd, "deployment.json");
  }
  ...
  return path.resolve(
    ...
    "deployments",
    `deployment.legacy.${network}.json`,
  );
}

function defaultShouldWriteLegacyEnvFile(options) {
  return (
...
function buildLegacyDeployRerunCommand(options) {
  const parts = [
    "pnpm run legacy:deploy -- --env-file",
    options.envFilePath,
    "--network",
    options.network,
  ];
...
async function runLegacyDeploy(options = {}) {
  ...
  if (requireEnvFile && !fs.existsSync(envFilePath)) {
    logger.fail(
      `Legacy Arena deploy env file not found at ${envFilePath}. Provide --env-file <path> for the target environment before rerunning pnpm run legacy:deploy.`,
    );
    return 1;
  }

  if (network !== "localhost" && !explicitDeployerKey) {
    logger.fail(
  ...
  const outputPath =
    options.outputPath || defaultLegacyDeployOutputPath(cwd, network);
  const writeEnv =
  ...
  const command = createCommand(
    ...
    {
      ...baseEnv,
      ARENA_DEPLOY_ENV_FILE: envFilePath,
      ARENA_DEPLOY_OUTPUT_PATH: outputPath,
      ARENA_DEPLOY_WRITE_ENV: writeEnv ? "1" : "0",
    },
  );
  ...
  if (!result || result.status !== 0) {
    logger.fail(
      `Legacy Arena deploy failed for network ${network}. Fix the failing command above, then rerun ${buildLegacyDeployRerunCommand({
        envFilePath,
        network,
        outputPath,
        writeEnv,
  ...
  logger.info(`Legacy Arena deploy artifact: ${outputPath}`);
  logger.pass(`Legacy Arena deploy completed for network ${network}.`);
  return 0;
}

async function main() {
  const exitCode = await runLegacyDeploy(parseArgs(process.argv.slice(2)));
