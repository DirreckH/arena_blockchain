...
它不是普通预测市场，也不是把问卷、交易和钱包登录简单拼接在一起的 demo。这个仓库要成立的是一条更严格的产品边界: 裁决层负责生成可验证结果，验证层围绕结果进行下注、结算与退款；开奖前公开的是进度，不是方向。
...
- 这是什么
  - 一个把 respondent 裁决层和 validation market 结算层放进同一条产品链路的 Web3 / AI 双层系统。
- 现在能跑什么
...
如果你第一次进入这个仓库，先把下面这几个事实看清楚:

- Arena 当前产品主线已经不是旧的 `PK` 合约 demo。
- 根目录仍然保留 legacy `contracts/Arena.sol` 与旧部署脚本，用于历史兼容，不应再视为新 validation 基线。
- 当前真正对齐产品叙事的是 proposition + adjudication + validation-chain 这一条 runtime。
- 当前 README 里的“已实现能力”只按现有 MVP 和 validation-chain 范围描述。
...
- 仅支持 `final`
- 一用户一市场一仓位
- validation-chain 当前只做 native asset stake / settlement / claim / refund

### 当前明确未完成或未宣称的能力

- survey / multi-question proposition
- hybrid proposition
- rolling proposition
- AMM / orderbook / odds
- multi-asset betting
