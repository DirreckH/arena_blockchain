...
describe('public results page', () => {
  ...
  it('renders live public settled archive rows from the real feed', async () => {
    ...
    expect(await screen.findByText('真实公开结果命题 A')).toBeInTheDocument()
    expect(await screen.findByText('真实公开结果命题 B')).toBeInTheDocument()
    expect(await screen.findByText('支持')).toBeInTheDocument()
    expect(await screen.findByText('会改善')).toBeInTheDocument()
    expect(await screen.findByText('胜出占比 62.5%')).toBeInTheDocument()
    expect(await screen.findByText('未附链上证据')).toBeInTheDocument()
  })

  it('shows a degraded source badge when the settled archive feed falls back to demo data', async () => {
    getPublicSettledResultsFeed.mockResolvedValue({
      sourceMode: 'mixed',
      data: {
        totalCount: 1,
